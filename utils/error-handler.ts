...
export class ErrorHandler {
    static handleError(error: Error | string, context?: string): ErrorInfo {
        ...
        } else if (errorMessage.includes('API error') || errorMessage.includes('HTTP')) {
            return this.handleAPIError(errorMessage, context);
        } else if (errorMessage.includes('text provided') || errorMessage.includes('too long')) {
    ...
    static showSimpleNotification(message: string, type: 'error' | 'warning' | 'info' = 'error'): void {
        const duration = type === 'error' ? 8000 : 5000;
        new Notice(message, duration);
    }

    static validateApiKey(service: string, apiKey: string): { valid: boolean; message?: string } {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                ErrorHandler.showSimpleNotification(
                    `Attempt ${attempt} failed, retrying... (${maxRetries - attempt} attempts left)`,
                    'warning'
                );
    ...
    static isRetryableError(error: Error): boolean {
        ...
        if (message.includes('rate limit') || message.includes('server error') || message.includes('503') || message.includes('502')) {
            return true;
        }
        
        // Configuration and validation errors are not retryable
        if (message.includes('api key') || message.includes('not configured') || message.includes('validation')) {
            return false;
        }
