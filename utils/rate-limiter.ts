...
export class RateLimiter {
    ...
    async checkRateLimit(): Promise<{ canProceed: boolean; waitTime: number }> {
        ...
        const waitTime = oldestRequest + this.config.cooldownPeriod! - now;
    ...
    async waitAndProceed(): Promise<void> {
        ...
        if (!canProceed && waitTime > 0) {
            console.log(`${this.serviceName}: Rate limiting - waiting ${waitTime}ms`);
     ...
     */
    getStatus(): { requestsInWindow: number; maxRequests: number; resetTime: number } {
        const now = Date.now();
        const windowStart = now - this.config.cooldownPeriod!;
        const requestsInWindow = this.requestTimes.filter(time => time > windowStart).length;
        const oldestRequest = Math.min(...this.requestTimes.filter(time => time > windowStart));
        const resetTime = oldestRequest + this.config.cooldownPeriod!;
